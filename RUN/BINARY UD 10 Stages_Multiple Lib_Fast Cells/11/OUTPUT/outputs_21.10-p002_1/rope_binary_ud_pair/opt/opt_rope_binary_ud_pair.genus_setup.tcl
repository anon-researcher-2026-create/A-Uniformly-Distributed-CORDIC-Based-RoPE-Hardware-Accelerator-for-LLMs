...
################################################################################
if {[::legacy::find -design design:rope_binary_ud_pair] ne ""} {
  puts "** A design with the same name is already loaded. It will be removed. **"
  delete_obj design:rope_binary_ud_pair
}
...
# Design
################################################################################
read_netlist -top rope_binary_ud_pair OUTPUT/outputs_21.10-p002_1/rope_binary_ud_pair/opt/opt_rope_binary_ud_pair.v.gz
read_metric -id current OUTPUT/outputs_21.10-p002_1/rope_binary_ud_pair/opt/opt_rope_binary_ud_pair.metrics.json

phys::read_script OUTPUT/outputs_21.10-p002_1/rope_binary_ud_pair/opt/opt_rope_binary_ud_pair.g.gz
...
# slack
set _slk_ [::legacy::get_attribute slack design:rope_binary_ud_pair]
if {[regexp {^-?[0-9.]+$} $_slk_]} {
  set _slk_ [format %.1f $_slk_]
}
if {$_slk_ != "1463.3"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden slack: 1463.3,  current slack: $_slk_"
}
unset _slk_
# multi-mode slack
# tns
set _tns_ [::legacy::get_attribute tns design:rope_binary_ud_pair]
if {[regexp {^-?[0-9.]+$} $_tns_]} {
...
unset _tns_
# cell area
set _cell_area_ [::legacy::get_attribute cell_area design:rope_binary_ud_pair]
if {[regexp {^-?[0-9.]+$} $_cell_area_]} {
  set _cell_area_ [format %.0f $_cell_area_]
}
if {$_cell_area_ != "4296"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden cell area: 4296,  current cell area: $_cell_area_"
}
unset _cell_area_
# net area
set _net_area_ [::legacy::get_attribute net_area design:rope_binary_ud_pair]
if {[regexp {^-?[0-9.]+$} $_net_area_]} {
  set _net_area_ [format %.0f $_net_area_]
}
if {$_net_area_ != "1693"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden net area: 1693,  current net area: $_net_area_"
}
unset _net_area_
